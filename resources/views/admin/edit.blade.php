...
<div class="p-6 mt-20 max-w-4xl mx-auto">
  <div class="bg-white shadow rounded-2xl p-6">
    ...
    <form action="{{ route('admin.update', $salles) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-6">
      @csrf
        @method('put')
      ...
      <div>
        ...
        <input type="text" name="name" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ old('title', $salles->name) }}" required>
      </div>

      <!-- Emplacement -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Emplacement</label>
        <input type="text" name="location" min="1" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ old('title', $salles->location) }}" required>
      </div>

       <!-- Prix -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Prix (FCFA/heure)</label>
        <input type="number" name="price_per_hour" step="0.01" min="0" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ old('title', $salles->price_per_hour) }}" required>
      </div>

      <!-- Type de salle -->
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1">Nombres de personnes</label>
        <input type="number" name="capacity" step="0.01" min="0" class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent text-sm" value="{{ old('title', $salles->capacity) }}" required>
      </div>
